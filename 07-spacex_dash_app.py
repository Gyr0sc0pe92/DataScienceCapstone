...
def scatter(site1, site2):
    print(site1)
    print(site2)
    if site1 == 'All Sites':
        new_df = spacex_df
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= site2[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= site2[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    else:
        new_df = spacex_df[spacex_df["Launch Site"] == site1]
        new_df2 = new_df[new_df["Payload Mass (kg)"] >= site2[0]]
        new_df3 = new_df2[new_df["Payload Mass (kg)"] <= site2[1]]
        #new_df2 = new_df[new_df["Payload Mass (kg)"] >= site2[0] & new_df["Payload Mass (kg)"] <= site2[1]]
        fig2 = px.scatter(new_df3, y="class", x="Payload Mass (kg)", color="Booster Version Category")
    return fig2
